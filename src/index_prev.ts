...
const TweenProp = 'position';

export declare type TweenCallback = Nullable<(object: Tween) => void>

export declare type Tween = {
    object: Reel,
    property: typeof TweenProp,
    propertyBeginValue: number,
    target: number,
    easing: (position: number) => number,
    time: number,
    change: TweenCallback,
...
(async () => {
    ...
    (document.getElementById('view') as HTMLElement).appendChild(app.canvas);
    ...
    function startPlay() {
        ...
        for (let i = 0; i < reels.length; i++) {
            ...
            tweenTo(r, TweenProp, target, time, backout(0.5), null, i === reels.length - 1 ? reelsComplete : null);
        }
    }
    ...
    function tweenTo(object: Reel, property: typeof TweenProp, target: number, time: number, easing: (t: number) => number, onchange: TweenCallback, oncomplete: TweenCallback): Tween {
        const tween: Tween = {
            object,
            property,
